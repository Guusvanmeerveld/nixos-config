...
# This corresponds to the Updates tab in the web UI.
latest_commit = None
version_update = requests.get(
    f"https://services.lidarr.audio/v1/update/plugins?version={old_version}",
).json()
if version_update["available"]:
    new_version = version_update["updatePackage"]["version"]
    # Use the timestamp to find the latest commit corresponding to the release,
    # because the plugins branch does not have tags.
    latest_commit = requests.get(
        "https://api.github.com/repos/Lidarr/Lidarr/commits"
        + "?sha=plugins"
        + f"&until={version_update['updatePackage']['releaseDate']}"
    ).json()[0]["sha"]

print(f"Version update: {old_version} → {new_version}")
...
source_nix_hash, source_store_path = (
    subprocess.run(
        [
            ...
            "--unpack",
            "--print-path",
            f"https://github.com/Lidarr/Lidarr/archive/{latest_commit}.tar.gz",
        ],
        stdout=subprocess.PIPE,
        text=True,
        check=True,
...
package_dir = "./pkgs/lidarr-plugins"
package_file_name = "default.nix"
...
with tempfile.TemporaryDirectory() as work_dir:
    ...
    replace_in_file(
        ...
        {
            # NB unlike hashes,
            # versions are likely to be used in code or comments.
